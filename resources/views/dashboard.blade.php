...
@section('page')
    ...
    <div class="flex items-stretch w-full mt-5">
            ...
            @forelse($today_events as $today_event)
                <div class="inline-block w-full hover:bg-gray-200 border-b-2 p-5 flex justify-between">
                    <div class="pt-2">
                        ...
                        <span class="text-base ml-10 text-blue-600">{{$today_event->title}}</span>
        ...
        <div class="flex-1 bg-white w-6/12 max-h-screen overflow-auto">
            ...
            @forelse($next_events as $next_event)
                <div class="inline-block w-full hover:bg-gray-200 border-b-2 p-5 flex justify-between ">
                    ...
                    <div class="flex content-center flex-wrap">
                        @if($next_event->starts_at->format('Y-m-d') == \Carbon\Carbon::now()->addDay()->format('Y-m-d'))
                            <span class="p-1 bg-blue-600 rounded shadow text-white">Tomorrow</span>
                        @endif
                    </div>
                </div>
            @empty
